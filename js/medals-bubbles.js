...
function createBubbleChart(error, countries, continentNames) {
  ...
  function createCircles() {
    ...
    .enter()
    .append("circle")
    .attr("r", function(d) { return circleRadiusScale(d.medals); })
    ...
    function updateCountryInfo(country) {
      var info = "";
      if (country) {
        info = [country.CountryName, formatPopulation(country.medals)].join(": ");
      }
  ...
  function createForces() {
    ...
    function createPopulationForces() {
      ...
      return {
        ...
        y: d3.forceY(function(d) {
          return populationScaleY(d.medals);
        }).strength(forceStrength)
      };
    }
  ...
  function forceCollide(d) {
    return countryCenterGrouping() || populationGrouping() ? 0 : circleRadiusScale(d.medals) + 1;
  }
  ...
  function addGroupingListeners() {
    ...
    addListener("#population",      forces.medals);
